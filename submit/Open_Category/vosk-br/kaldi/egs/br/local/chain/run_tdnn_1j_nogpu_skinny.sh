...
if [ $stage -le 11 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 2 --cmd "$train_cmd" ${lores_train_data_dir} \
...
if [ $stage -le 13 ]; then
  ...
  relu-batchnorm-layer name=tdnn1 $tdnn_opts dim=512 input=input2
  ...
  tdnnf-layer name=tdnnf13 $tdnnf_opts dim=512 bottleneck-dim=96 time-stride=3
